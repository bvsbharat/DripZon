...
const ProductListCard: React.FC<ProductListCardProps> = ({ 
  ...
  return (
    <div
      onClick={onClick}
      className={`flex h-16 rounded-2xl cursor-pointer transition-all duration-300 overflow-hidden ${
        isSelected
          ? "bg-white/20 border border-white/30 shadow-lg"
          : "bg-white/5 hover:bg-white/10 border border-white/10 hover:shadow-md"
      }`}
    >
      <div className="w-16 h-full rounded-l-2xl overflow-hidden flex-shrink-0">
        <img
          src={product.image}
          alt={product.name}
