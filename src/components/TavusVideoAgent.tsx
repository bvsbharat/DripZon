...
const TavusVideoAgent: React.FC<TavusVideoAgentProps> = ({
  ...
  // Memory service removed - no initial memory fetch needed

  // Memory service removed - no search memories function needed

  // Memory service removed - no save memory function needed

  // Memory service removed - no initial memory context needed
  ...
  const personaId = import.meta.env.VITE_TAVUS_PERSONA_ID || "p7cb9fe20b76";
  ...
  const createTavusConversation = async () => {
    try {
      ...
      // Get base conversational context
      const baseContext = tavusConfig.getPromptText();

      // Add initial memory context if available
      let conversational_context = baseContext;
      if (initialMemoryContext && initialMemoryContext.hasHistory) {
        const memoryContextText = `\n\nIMPORTANT - User Memory Context:\nThis user has previous interaction history. Here are their preferences and past interactions: ${initialMemoryContext.preferences}\nTotal previous interactions: ${initialMemoryContext.memoryCount}\nUse this context to provide personalized recommendations and acknowledge their history when appropriate.`;
        conversational_context = baseContext + memoryContextText;
        console.log(
          "Added initial memory context to conversation:",
          memoryContextText
        );
      } else {
        conversational_context =
          baseContext +
          "\n\nThis appears to be a new user with no previous interaction history. Provide a warm welcome and help them discover products.";
      }
      ...
      const response = await fetch("https://tavusapi.com/v2/conversations", {
        ...
        body: JSON.stringify({
          ...
          conversational_context,
          custom_greeting:
            initialMemoryContext && initialMemoryContext.hasHistory
              ? "Hello! I'm Riya, your AI stylist agent. I remember our previous conversations and I'm excited to help you continue your shopping journey. "
              : "Hello, I'm Riya, your AI stylist agent. Welcome! I'm here to help you discover amazing products. ",
          properties: {
        ...
        try {
          ...
          switch (name) {
            case "search_products": {
              ...
              // Memory service removed - no query saving needed
              ...
              const extractContextFromQuery = (query: string) => {
                ...
                const priceMatch = lowerQuery.match(
                  /under\s*\$?(\d+)|below\s*\$?(\d+)|less than\s*\$?(\d+)/
                );
                if (priceMatch) {
                ...
                if (
                  ...
                  lowerQuery.includes("classic") ||
                  lowerQuery.includes("timeless") ||
                  lowerQuery.includes("traditional")
                ) {
                  extractedContext.style = "classic";
                } else if (
                  lowerQuery.includes("trendy") ||
                  lowerQuery.includes("fashionable") ||
                  lowerQuery.includes("stylish")
                ) {
                  extractedContext.style = "trendy";
                } else if (
                  lowerQuery.includes("vintage") ||
                  lowerQuery.includes("retro")
                ) {
              ...
              const selectBestProduct = (
                ...
                products.forEach((product) => {
                  ...
                  if (context.color) {
                    ...
                    if (
                      colorKeywords === "red" &&
                      (productText.includes("crimson") ||
                        productText.includes("rust"))
                    )
                      score += 70;
                    if (
                      colorKeywords === "blue" &&
                      (productText.includes("navy") ||
                        productText.includes("azure"))
                    )
                      score += 70;
                    if (
                      colorKeywords === "white" &&
                      (productText.includes("ivory") ||
                        productText.includes("cream"))
                    )
                      score += 70;
                  }
                  ...
                  if (context.priceRange) {
                    ...
                    if (context.priceRange === "budget" && price < 100)
                      score += 20;
                    else if (
                      context.priceRange === "mid-range" &&
                      price >= 100 &&
                      price <= 500
                    )
                      score += 20;
                    else if (context.priceRange === "luxury" && price > 500)
                      score += 20;
                  ...
                  .map((product) => {
                    ...
                    searchTerms.forEach((term) => {
                      ...
                      if (
                        searchableText.includes(
                          term.substring(0, Math.max(3, term.length - 2))
                        )
                      ) {
                        score += 5;
                      }
                    ...
                    if (context.occasion) {
                      ...
                      if (occasion === "business" || occasion === "formal") {
                        if (
                          searchableText.includes("blazer") ||
                          searchableText.includes("formal") ||
                          searchableText.includes("professional") ||
                          searchableText.includes("elegant")
                        ) {
                          score += 15;
                        }
                      }
                      if (occasion === "casual" || occasion === "weekend") {
                        if (
                          searchableText.includes("casual") ||
                          searchableText.includes("denim") ||
                          searchableText.includes("trendy") ||
                          searchableText.includes("comfortable")
                        ) {
                          score += 15;
                        }
                      }
                      if (occasion === "evening") {
                        if (
                          searchableText.includes("elegant") ||
                          searchableText.includes("sophisticated") ||
                          searchableText.includes("luxurious") ||
                    ...
                    if (context.priceRange) {
                      ...
                      if (
                        priceRange.includes("affordable") ||
                        priceRange.includes("under")
                      ) {
                        if (product.price < 1500) score += 10;
                      }
                      if (
                        priceRange.includes("luxury") ||
                        priceRange.includes("premium")
                      ) {
                        if (product.price > 2000) score += 10;
                      }
                      if (priceRange.includes("mid-range")) {
                        if (product.price >= 1000 && product.price <= 2500)
                          score += 10;
                      }
                    }
                    ...
                    if (context.style) {
                      ...
                      if (style === "modern" || style === "contemporary") {
                        if (
                          searchableText.includes("modern") ||
                          searchableText.includes("contemporary") ||
                      ...
                      if (style === "classic" || style === "timeless") {
                        if (
                          searchableText.includes("classic") ||
                          searchableText.includes("timeless") ||
                    ...
                    if (context.color) {
                      ...
                      if (
                        color === "red" &&
                        (searchableText.includes("crimson") ||
                          searchableText.includes("rust"))
                      ) {
                        score += 35;
                      }
                      if (
                        color === "blue" &&
                        (searchableText.includes("navy") ||
                          searchableText.includes("azure"))
                      ) {
                        score += 35;
                      }
                      if (
                        color === "white" &&
                        (searchableText.includes("ivory") ||
              ...
              if (searchResults.length > 0) {
                ...
                const topRecommendations = searchResults
                  .slice(0, 5)
                  .map((product) => ({
                    id: product.id,
                    name: product.name,
                ...
                result = `Perfect! I found the ideal match for you: **${
                  selectedProduct.name
                }** (ID: ${selectedProduct.id}) - $${selectedProduct.price}. ${
                  selectedProduct.description
                }${contextualInfo}. This is my top recommendation based on your preferences${
                  searchResults.length > 1
                    ? ` from ${searchResults.length} matching products`
                    : ""
                }. Would you like to add it to your cart or see more details?`;

                console.log("AI Selected Best Product:", {
                ...
                if (extractedContext.color || mergedContext.color) {
                  const colorQuery =
                    extractedContext.color || mergedContext.color;
                ...
                // Remove duplicates
                const uniqueFallbackProducts = Array.from(
                  new Set(fallbackProducts.map((p) => p.id))
                ...
                if (uniqueFallbackProducts.length > 0) {
                  ...
                  const bestFallback =
                    uniqueFallbackProducts.length > 1
                      ? selectBestProduct(
                          uniqueFallbackProducts,
                          mergedContext,
                          query
                        )
            ...
            case "add_to_cart": {
              ...
              // Memory service removed - no product memory saving needed
              ...
              if (matchedCategory) {
                ...
                if (categoryProducts.length > 0) {
                  ...
                  if (parsedArgs.preference) {
                    const preferredProduct = categoryProducts.find(
                      (product) =>
                        product.name
                          .toLowerCase()
                          .includes(parsedArgs.preference.toLowerCase()) ||
                        product.description
                          ?.toLowerCase()
                          .includes(parsedArgs.preference.toLowerCase())
                    );
                  ...
                  console.log(
                    `Category switched to ${matchedCategory}, selected product: ${selectedCategoryProduct.name}`
                  );
                }
            ...
            case "show_kart": {
              ...
              if (currentCartState.isEmpty) {
                ...
                result = `I've opened your cart for you. You have ${
            ...
            case "trigger_checkout": {
              ...
              // Get current cart state to verify items exist
              const currentCartState = getCurrentCartState();
              ...
              result = `Processing checkout for ${
                currentCartState.totalItems
              } items totaling $${currentCartState.totalPrice.toFixed(
                2
              )}. Please wait...`;
              break;
            }
