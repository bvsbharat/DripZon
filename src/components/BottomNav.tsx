...
import { Home, Camera, Settings, ShoppingBag, Check, User, ArrowLeft } from "lucide-react";
...
const BottomNav: React.FC<BottomNavProps> = ({
  ...
  // Show different nav items based on current view
  const getNavItems = () => {
    if (currentView === "settings") {
      return [
        { id: "home", icon: ArrowLeft, label: "Back" },
      ...
      <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-80 h-16 bg-white/10 backdrop-blur-xl border border-white/20 rounded-t-3xl z-30 shadow-2xl">
        <div className="flex items-center justify-around h-full px-4">
          {navItems.map((item) => {
            ...
            // Special handling for back button on settings page
            const handleClick = () => {
              if (currentView === "settings" && item.id === "home") {
                console.log('🔍 BottomNav - Navigating back to home from settings');
                setCurrentView("home");
              } else {
