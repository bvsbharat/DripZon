...
const TryOnDisplay: React.FC = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `AI_TryOn_${
        selectedProduct?.name?.replace(/\s+/g, "_") || "Result"
      }_${Date.now()}.png`;
  ...
  const handleShare = (platform?: string) => {
    ...
    const shareText = `Check out my AI virtual try-on result with ${
      selectedProduct?.name || "this amazing outfit"
    }! #AIFashion #VirtualTryOn #Style`;
  ...
  return (
    <>
        ...
        <>
          ...
          <div className="relative w-full h-full flex items-center justify-center group">
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between items-start opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={isGeneratingVideo || !tryOnResult}
                ...
                {/* Play Video Button - only show if video is available */}
                {videoResult && (
                  <button
            ...
            <button
              ...
              className="absolute top-4 right-4 bg-black/60 backdrop-blur-xl p-2 rounded-full text-white hover:bg-black/80 transition-colors z-20 shadow-lg"
              title="Clear Try-On Result"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {showVideoModal && videoResult && (
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => setShowVideoModal(false)}
                className="absolute top-4 right-4 bg-black/60 backdrop-blur-xl p-2 rounded-full text-white hover:bg-black/80 transition-colors z-10"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              {/* Video Info */}
              <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-xl rounded-lg p-3">
                <p className="text-white text-sm font-medium">
                  AI Generated Video
                </p>
                <p className="text-white/70 text-xs">{selectedProduct?.name}</p>
      ...
      {/* Video Error Display */}
      {videoError && (
        <div className="absolute top-16 left-0 right-0 mx-auto w-max bg-red-500/70 text-white text-sm px-4 py-2 rounded-lg z-10">
          {videoError}
        </div>
      )}
    </>
